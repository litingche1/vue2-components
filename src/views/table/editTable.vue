...
<template>
  <div class="edit-table-wrap">
    <h1>编辑表格</h1>
    <div>
      1.首先在ux-grid上绑定 keep-source属性，edit-config = {trigger: 'click',//
      点击触发， mode: 'row' // 作用点在谁身上 row表示行，cell: 表示单元格}<br />
      2.然后再列上需要定义edit-render属性表示是可以编辑的列，，添加了呢，然后头部header会出现有编辑图形的icon啦，代表是可以编辑的行<br />
      3.设置 keep-source
      开启保持原始值状态，对于某些需要局部保存的场景，可以在数据保存完成后调用
      reloadRow 方法加载行数据并恢复到初始状态 <br />
      4.可以支持大量的列<br />
